...
4. Use the returned checkpoint with `preview`, `tools`, `accept-tools`, and the existing `next → execute once → accept` loop. Refresh the callable tool check in each new chat before new operations; for the confirmed Runlayer catalog names use `tools CHECKPOINT --runlayer-catalog` as documented in execution.md; resolve pending receipts first. Generated accept commands use the absolute runner path.
